...
create_marray <- function(rel_pre, rel_pos, rel_yrs, s, h, l) {
  ...
  for (y in 1:yrs) {
    ...
    } # Assume no tag loss immediately after tagging(?)
  }
  ...
  for (y in 1:(yrs - 1)) {
    for (j in (y + 1):yrs) {
      for (t in 1:2){
      pi[pre_idx[y], j, t] <- prod(s[y:(j - 1)] * (1 - h[y:(j - 1)])) * h[j] * l[j, t] # add term for harvest in winter...
    }     # past years' survival rate * past year's probability of avoiding harvest * ANNUAL TAG LOSS OF 0.10 *
