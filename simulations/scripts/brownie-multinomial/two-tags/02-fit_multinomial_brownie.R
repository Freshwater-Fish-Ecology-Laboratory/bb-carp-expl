...
if (drac == TRUE) {
  source("02-fn_simulate_marray.R")
  ...
  source("simulations/codes/R/brownie/multinomial/two-tags/02-fn_simulate_marray.R")
  ...
  h <- rep(0.20, yrs) # harvest probability
  s <- rep(0.2, yrs) # survival probability
  l <- matrix(c(rep(0.7, yrs), rep(1, yrs)), nrow = yrs, ncol = 2) # reporting probability
...
for (i in 1:nsim) {
  ...
  sim_input[[i]] <- list(yrs = yrs, s = s, h = h, l = l, 
...
if (drac == TRUE) {
  saveRDS(out, paste("output_2tag_", scn, ".rds", sep = ""))  
} else {
  saveRDS(out, paste("simulations/outputs/02-tags/output_2tag_", scn, ".rds", sep = ""))  
}
