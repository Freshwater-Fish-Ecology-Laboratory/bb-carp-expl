# JAGS model: multinomial likelihood and 2 tag types with winter harvest rate
model {
  ...
  for (y in 1:(Y - 1)) {
    for (j in (y + 1):Y) {
      for (t in 1:2) {
        pi[pre_idx[y], j, t] <- prod(s[y:(j - 1)] * (1 - h[y:(j - 1)])) * (1 - w[y:(j - 1)]) * h[j] * l[j, t]
      } # ADDED TAG LOSS RATE
    }
  }
  ...
  for (r in 1:R) {
    for(t in 1:2) {
      marr[r, , t] ~ dmulti(pi[r, , t], rel[r, t])
    }
  }
}
